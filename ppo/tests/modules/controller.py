...
from models.qfs_sac import QFSSAC, QFS_CS_SAC, QFS_LS_SAC
...
def train_qfs_sac(seed:int, total_time_steps:int, save_dir:str, log_dir:str, mkenv_func : Callable, env_args:dict, alg_args:dict):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    n_envs = 1  # 원하는 env 개수
    if 'n_envs' in env_args:
    ...
    model = QFSSAC("MlpPolicy", vec_env, verbose=0, tensorboard_log=log_dir,
                    seed=seed, **sac_args)
    model.learn(total_timesteps=total_time_steps, tb_log_name=f"QFS_SAC_{seed}")
    model.save(f"{save_dir}qfs_sac_{seed}")
    vec_env.close()
    del model

def train_qfs_cs_sac(seed:int, total_time_steps:int, save_dir:str, log_dir:str, mkenv_func : Callable, env_args:dict, alg_args:dict):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    n_envs = 1
    if 'n_envs' in env_args:
    ...
    sac_args = {k: v for k, v in env_args.items() if k != "n_envs"}
    sac_args.update(alg_args)
    model = QFS_CS_SAC("MlpPolicy", vec_env, verbose=0, tensorboard_log=log_dir,
                    seed=seed, **sac_args)
    model.learn(total_timesteps=total_time_steps, tb_log_name=f"QFS_CS_SAC_lamS{alg_args['qfs_lamS']}_lamT{alg_args['qfs_lamT']}_{seed}")
    model.save(f"{save_dir}qfs_cs_sac_lamS{alg_args['qfs_lamS']}_lamT{alg_args['qfs_lamT']}_{seed}")
    vec_env.close()
    del model
